...
from helpers.custom_layer import KMaxPooling
...
class DocumentModel:
    ...
    def __init__(self, vocab_size,
        ...
        if vocab_size != len(word_index):
            print("Vocab Size = {}  and the index of vocabulary words passed has {} words".format(vocab_size,
                                                                                                  len(word_index)))
    ...
    def _build_model(self):
        ...
        sentence = Lambda(lambda x: x[:, 0: 300, :])(z)
        ...
        self.model = Model(sequence_input, model_output)

    def save_model(self, file_name):
        model_params = {}
        for key in self.__dict__.keys():
            if key not in ['model', 'embedding_weights']:
    ...
    def get_document_model(self):
        return Model(inputs=self.model.input,
                     outputs=self.model.get_layer('document_embedding').output)

    def get_sentence_model(self):
        return Model(inputs=self.model.input,
                     outputs=self.model.get_layer('sentence_embeddings').output)
    ...
    def load_model_weights(self, model_weights_filename):
        self.model.load_weights(model_weights_filename, by_name=True)
